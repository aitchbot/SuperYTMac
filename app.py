...
def _detectar_deno():
    ...
    Se busca primero en el PATH y, si todavía no está disponible ahí (recién instalado
    con Homebrew, antes de abrir una terminal nueva), en las rutas típicas de Homebrew
    (Apple Silicon usa /opt/homebrew, Intel usa /usr/local).
    """
    ruta = shutil.which("deno")
    if ruta:
        return ruta
    for candidato in ("/opt/homebrew/bin/deno", "/usr/local/bin/deno"):
...
class SuperYT(tk.Tk):
    ...
    def _agregar_menu_contextual(widget):
        """Agrega Cortar/Copiar/Pegar/Seleccionar todo al clic derecho (Tkinter no lo trae
        por defecto). En macOS el clic derecho puede disparar <Button-2> (mouse/trackpad
        heredado de un solo botón) o <Button-3> (mouse de dos botones real), y el gesto
        clásico con trackpad/mouse de un botón es Control+clic — se atan los tres."""
        ...
        widget.bind("<Button-2>", mostrar)
        widget.bind("<Button-3>", mostrar)
        widget.bind("<Control-Button-1>", mostrar)
    ...
    def _dialogo_seleccion(self, info, resultado, evento):
        ...
        scroll.pack(side="right", fill="y")
        # En macOS el delta de <MouseWheel> viene en unidades chicas (no multiplos de 120
        # como en Windows), asi que alcanza con mirar el signo para decidir la direccion.
        dlg.bind_all("<MouseWheel>", lambda e: canvas.yview_scroll(-1 if e.delta > 0 else 1, "units"))
